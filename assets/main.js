...
// Update this after your first Worker deploy (Cloudflare → Workers → calvinbot-api → preview/dev)
const CALVINBOT_API = 'https://calvinbot-api.YOUR_ACCOUNT.workers.dev';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function navigateTo(hash) {
    ...
    if (target) {
      ...
      // Reveal elements inside the newly active section
  ...
  // ─── Toggle particles on low-power devices ───
  ...
  (function initCalvinBot() {
    ...
    async function handleSubmit(e) {
      ...
      try {
        ...
        const type = data.fromAI ? 'bot' : 'bot';
        addMessage(data.answer || 'No response received.', type);
      } catch (err) {
        removeTyping();
